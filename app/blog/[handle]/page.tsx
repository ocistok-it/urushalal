  ...
  return (
    <div className="min-h-screen bg-white">
      <div className="pt-24 md:pt-32 pb-16 px-4 max-w-3xl mx-auto">
        <Link
          href="/blog"
          className="inline-flex items-center gap-1.5 text-sm text-teal-600 hover:text-teal-700 font-medium mb-8 transition-colors"
        >
          <svg
            className="w-4 h-4"
            fill="none"
        ...
        {error ? (
          ...
          <article>
            ...
            {article.tag?.length > 0 && (
              <div className="flex flex-wrap gap-2 mb-8">
                {article.tag.map((t) => (
                  <span
                    key={t}
                    className="px-3 py-1 rounded-full text-xs font-medium bg-teal-50 text-teal-700"
                  >
                    {t}
                  </span>
                ))}
            ...
            <div
              className="prose prose-gray prose-img:rounded-xl prose-a:text-teal-600 hover:prose-a:text-teal-700 max-w-none"
                    ...
                    meta={
                      <>
                        ...
                        <span className="text-gray-400">
                          {formatDate(new Date(rec.date).toLocaleString())}
                        </span>
                      </>
                    }
                  />
